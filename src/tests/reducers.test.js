...
describe('reducer', () => {
  test('should return the initial state', () => {
    const initilaState = { missions: [mission1, mission2], people: [] };
    deepFreeze(initilaState);
    expect(reducer(initilaState)).toEqual(initilaState);
  });
  test('should remove a mission', () => {
    const initilaState = { missions: [mission1, mission2], people: [] };
    const expectedState = { missions: [mission2], people: [] };
    deepFreeze(initilaState);
    expect(reducer(initilaState, actions.removeMission(mission1.id))).toEqual(expectedState);
  });
  test('should remove   missions', () => {
    const selectedMission2 = { ...mission2, isSelected: true };
    const initilaState = { missions: [mission1, selectedMission2], people: [] };
    const expectedState = { missions: [mission1], people: [] };
    deepFreeze(initilaState);
    expect(reducer(initilaState, actions.removeMissions())).toEqual(expectedState);
  });
